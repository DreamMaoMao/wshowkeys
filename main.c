...
static int get_char_width(char *name) {
	if (strstr("⏎ ␣ ⇦ ⇧ ⇨ ", name)) return 4;
	if (strstr("⌫ F12 F10 F11  Esc ", name)) return 5;
	if (strstr("ijl-\\*()!,./[]{}012", name)) return 1;
	if (strstr("-=+abcdefghkmnoqrstuvwxyz@#$%^&?><", name)) return 2;
	if (strstr("F1F2F3F4F5F6F7F8F93456789", name)) return 3;
	if (strstr(" Ctrl+", name)) return 8;
	if (strstr(" Alt+", name)) return 6;
	if (strstr(" Shift+", name)) return 10;
	if (strstr(" Super+", name)) return 10;
	if (strstr("Tab ", name)) return 10;
	if (strstr("Caps ", name)) return 8;
	return strlen(name);
}

static void custome_key_name(char *name) {
	if (strcmp(name, "Return") == 0) { strcpy(name, "⏎ "); }
	else if (strcmp(name, "space") == 0) { strcpy(name, "␣ "); }
	else if (strcmp(name, "Escape") == 0) { strcpy(name, " Esc "); }
	else if (strcmp(name, "Control_L") == 0) { strcpy(name, " Ctrl+"); }
	else if (strcmp(name, "Control_R") == 0) { strcpy(name, " Ctrl+"); }
	else if (strcmp(name, "Alt_L") == 0 || strcmp(name, "Meta_L") == 0) { strcpy(name, " Alt+"); }
	else if (strcmp(name, "Alt_R") == 0 || strcmp(name, "Meta_R") == 0) { strcpy(name, " Alt+"); }
	else if (strcmp(name, "Shift_L") == 0) { strcpy(name, " Shift+"); }
	else if (strcmp(name, "Shift_R") == 0) { strcpy(name, " Shift+"); }
	else if (strcmp(name, "Super_L") == 0) { strcpy(name, " Super+"); }
	else if (strcmp(name, "Super_R") == 0) { strcpy(name, " Super+"); }
	else if (strcmp(name, "Tab") == 0) { strcpy(name, "Tab "); }
	else if (strcmp(name, "backslash") == 0) { strcpy(name, "\\"); }
	else if (strcmp(name, "BackSpace") == 0) { strcpy(name, "⌫ "); }
	else if (strcmp(name, "Caps_Lock") == 0) { strcpy(name, "Caps "); }
	else if (strcmp(name, "Left") == 0) { strcpy(name, "⇦ "); }
	else if (strcmp(name, "Up") == 0) { strcpy(name, "⇧ "); }
	else if (strcmp(name, "Down") == 0) { strcpy(name, "⇩ "); }
	else if (strcmp(name, "Right") == 0) { strcpy(name, "⇨ "); }
	else if (strcmp(name, "KP_Insert") == 0) { strcpy(name, "0"); }
	else if (strcmp(name, "KP_End") == 0) { strcpy(name, "1"); }
	else if (strcmp(name, "KP_Down") == 0) { strcpy(name, "2"); }
	else if (strcmp(name, "KP_Next") == 0) { strcpy(name, "3"); }
	else if (strcmp(name, "KP_Left") == 0) { strcpy(name, "4"); }
	else if (strcmp(name, "KP_Begin") == 0) { strcpy(name, "5"); }
	else if (strcmp(name, "KP_Right") == 0) { strcpy(name, "6"); }
	else if (strcmp(name, "KP_Home") == 0) { strcpy(name, "7"); }
	else if (strcmp(name, "KP_Up") == 0) { strcpy(name, "8"); }
	else if (strcmp(name, "KP_Prior") == 0) { strcpy(name, "9"); }
	else if (strcmp(name, "KP_Delete") == 0) { strcpy(name, "."); }
	else if (strcmp(name, "KP_Enter") == 0) { strcpy(name, "⏎ "); }
}
...
static void get_text_metrics(cairo_t *cairo, const char *font, const char *text,
                             int scale, int *width, int *height, int *baseline) {
...
static void render_mods_only(cairo_t *cairo, struct wsk_state *state, int scale,
	...
	const char *mod_names[] = {"Ctrl", "Super", "Alt", "Shift"};
	bool mod_active[] = {
		state->ctrl_l_hold || state->ctrl_r_hold,
	...
	for (int i = 0; i < num_mods; i++) {
		...
		cairo_set_source_u32(cairo, mod_active[i] ? state->background : state->foreground);
...
static void render_keys_only(cairo_t *cairo, struct wsk_state *state, int scale,
	...
	while (key) {
		...
		if (special) {
			...
			get_text_size(cairo, state->font, &tw, &th, NULL, scale, "%s", name);
...
static void render_mods_frame(struct wsk_state *state) {
	if (!state->mods_surface || !state->mods_width || !state->mods_height) return;
	...
	if (w != state->mods_width || h != state->mods_height) {
		zwlr_layer_surface_v1_set_size(state->mods_layer_surface,
		                               w / scale, h / scale);
		wl_surface_commit(state->mods_surface);
	} else {
		if (!create_buffer(state->shm, &buffer_mods,
		                   state->mods_width * scale,
		...
		wl_surface_damage_buffer(state->mods_surface, 0, 0,
		                         state->mods_width, state->mods_height);
		wl_surface_commit(state->mods_surface);
...
static void render_keys_frame(struct wsk_state *state) {
	if (!state->keys_surface || !state->keys_width || !state->keys_height) return;
	int scale = state->output ? state->output->scale : 1;
	...
	if (new_h != state->keys_height || new_w != state->keys_width) {
		...
		if (!create_buffer(state->shm, &buffer_keys,
		                   state->keys_width * scale,
		...
		wl_surface_damage_buffer(state->keys_surface, 0, 0,
		                         state->keys_width, state->keys_height);
		wl_surface_commit(state->keys_surface);
...
static void surface_enter(void *data, struct wl_surface *wl_surface,
	...
	while (out && out->output != output) out = out->next;
...
static void registry_global(void *data, struct wl_registry *wl_registry,
	...
	if (strcmp(interface, wl_compositor_interface.name) == 0) {
		...
		output->output = wl_registry_bind(wl_registry, name,
...
static void attach_to_last(struct wsk_state *state, struct wsk_keypress *key) {
	...
	while (*attach) attach = &(*attach)->next;
	*attach = key;
}
			...
			if (strcmp(state->prev_combination_keye, "") != 0 &&
				...
				if (state->combination_keye_repetition > 2) {
					int add_charnum = caculat_add_charnum_of_int(
						state->combination_keye_repetition);
					attach_repeat_flag(state,
						state->combination_keye_repetition, add_charnum);
				}
			} else {
...
static uint32_t parse_color(const char *color) {
	...
	if (strlen(color) == 6) res = (res << 8) | 0xFF;
	return res;
}
...
int main(int argc, char *argv[]) {
	...
	while ((c = getopt(argc, argv, "hb:f:s:F:t:a:m:o:l:")) != -1) {
		...
		case 'f': state.foreground = parse_color(optarg); break;
		case 's': state.specialfg = parse_color(optarg); break;
		case 'F': state.font = optarg; break;
		case 't': state.timeout = atoi(optarg); break;
		case 'a':
	...
	state.libinput = libinput_udev_create_context(&libinput_impl,
	                                              &state.devmgr, state.udev);
	udev_unref(state.udev);
	if (!state.libinput) { ret = 1; goto exit; }

	state.xkb_context = xkb_context_new(XKB_CONTEXT_NO_FLAGS);
	if (!state.xkb_context) { ret = 1; goto exit; }

	state.display = wl_display_connect(NULL);
	if (!state.display) { ret = 1; goto exit; }
	...
	uint32_t mods_surf_h = mods_pixel_h / scale;
	if (mods_surf_w < 1) mods_surf_w = 1;
	if (mods_surf_h < 1) mods_surf_h = 1;

	/* 创建修饰键 surface，固定大小 */
	state.mods_surface = wl_compositor_create_surface(state.compositor);
	assert(state.mods_surface);
	wl_surface_add_listener(state.mods_surface, &wl_surface_listener, &state);
	...
	struct wl_region *mods_input = wl_compositor_create_region(state.compositor);
	wl_surface_set_input_region(state.mods_surface, mods_input);
	wl_region_destroy(mods_input);
	zwlr_layer_surface_v1_add_listener(state.mods_layer_surface,
	                                   &mods_layer_surface_listener, &state);
	zwlr_layer_surface_v1_set_anchor(state.mods_layer_surface, anchor);
	zwlr_layer_surface_v1_set_margin(state.mods_layer_surface,
	                                 margin, margin, margin, margin);
	zwlr_layer_surface_v1_set_exclusive_zone(state.mods_layer_surface, -1);
	zwlr_layer_surface_v1_set_size(state.mods_layer_surface,
	                               mods_surf_w, mods_surf_h);
	wl_surface_commit(state.mods_surface);

	/* 创建按键 surface，底部 margin 需要额外增加修饰键高度 + 间隙 */
	const int keys_gap = 4;   // 按键行与修饰键之间的间距
	...
	struct wl_region *keys_input = wl_compositor_create_region(state.compositor);
	...
	/* bottom margin = 基础 margin + 修饰键面板高度 + 间隙 */
	zwlr_layer_surface_v1_set_margin(state.keys_layer_surface,
	                                 margin, margin,
	                                 margin + mods_surf_h + keys_gap,
	                                 margin);
	...
	while (state.run) {
		...
		if (pollfds[0].revents & POLLIN) {
			if (libinput_dispatch(state.libinput) != 0) break;
			struct libinput_event *event;
...
exit:
	...
	if (state.mods_surface) wl_surface_destroy(state.mods_surface);
	if (state.keys_surface) wl_surface_destroy(state.keys_surface);
