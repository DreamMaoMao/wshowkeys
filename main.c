...
/* -------- Pango 辅助 -------- */
static void get_text_size(cairo_t *cairo, const char *font,
                          int *width, int *height, int *baseline,
                          int scale, const char *fmt, ...) {
    char text[1024];
    va_list args;
    va_start(args, fmt);
    ...
    if (baseline) *baseline = pango_layout_get_baseline(layout) / PANGO_SCALE;
...
static void render_mods_frame(struct wsk_state *state) {
    if (!state->show_mods || !state->mods_surface || !state->mods_width || !state->mods_height)
        return;
